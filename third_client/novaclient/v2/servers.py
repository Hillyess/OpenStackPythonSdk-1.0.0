...
class ServerManager(base.BootingManagerWithFind):
    ...
    def _boot(self, resource_url, response_key, name, image, flavor,
              ...
              return_reservation_id=None, return_raw=False, min_count=None,
        ...
        if return_reservation_id:
            body["server"]["return_reservation_id"] = return_reservation_id
        if key_name:
    ...
    #modify on 2016-05-20  by xwx344867 
    def list(self, detailed=True, search_opts=None, marker=None, limit=None, tag=None, tag_any=None):
        """
    ...
    def show_ip_details(self, server, networkname):
        """
        :param server:The :class:`Server` (or its ID) for which
                       the IP adresses are to be returned
        :param networkname:Network name
        :return: An instance of novaclient.base.DictWithMeta
        """
        resp, body = self.api.client.get("/servers/%s/ips/%s" % (base.getid(server),networkname))
    ...
    def create(self, name, image, flavor, meta=None, files=None,
               return_reservation_id=None, min_count=None,
               max_count=None, security_groups=None, userdata=None,
               key_name=None, availability_zone=None,block_device_mapping_v2=None,
        ...
        boot_kwargs = dict(
            ...
            return_reservation_id=return_reservation_id, min_count=min_count,
        ...
        response_key = "reservation_id" if return_reservation_id else "server"
#        response_key = "server"
    ...
    def list_meta(self, server):
        """
        List a servers metadata
        :param server: The :class:`Server` or Server id
        :return:
        """
        resp, body = self.api.client.get("/servers/%s/metadata" %
                                         base.getid(server))
        return base.DictWithMeta(body['metadata'], resp)
    ...
    def update_meta(self, server, metadata):
        """
        Update a servers metadata
        :param server: The :class:`Server` to update metadata to
        :param metadata: A dict of metadata to update to the server
        """
        body = {'metadata': metadata}
        return self._update("/servers/%s/metadata" % base.getid(server),
                            body, "metadata")
    ...
    def get_meta_item(self, server, key):
        """
        Get an item of server metadata
        :param server:The :class:`Server` to get metadata to
    ...
    def delete_meta_item(self, server, key):
        """
        Delete an item of server metadata
        :param server:The :class:`Server` to get metadata to
        :param key: metadata key
        """
    ...
    def interface_show(self, server, port_id):
        """
        Show a network_interface from an instance.
        :param server: The :class:`Server` (or its ID) to detach from.
        :param port_id: The port
        """
        return self._get('/servers/%s/os-interface/%s' %
                         (base.getid(server), port_id), 'interfaceAttachment')
